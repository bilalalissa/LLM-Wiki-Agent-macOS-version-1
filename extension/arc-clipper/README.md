...
The popup shows a progress bar while media downloads are attempted. Review the title, text length, media count, and per-media download status before clicking `Submit to vault`. Enter optional comma-separated tags, for example `meeting, research, ai/agents`; the agent saves them in the Markdown frontmatter and the source summary.
